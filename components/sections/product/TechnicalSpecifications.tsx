...
const headerCellClass =
  "shrink-0 border-r border-neutral-3 px-16 py-6 text-p2 font-semibold text-white last:border-r-0 md:px-24";
const bodyCellClass =
  "shrink-0 border-r border-neutral-2 px-16 py-12 text-p2 text-neutral-9 last:border-r-0 md:px-32 md:py-20";
              ...
              {data.tableA.map((row, index) => (
                ...
                >
                  <div
                    className={cn(
                      bodyCellClass,
                      "sticky left-0 w-200 bg-neutral-1 font-semibold text-white md:w-360"
                    )}
                  >
                    {row.label}
                  </div>
                  <div className={cn(bodyCellClass, "flex-1")}>{row.value}</div>
                </div>
              ))}
            </div>
          </Reveal>
              ...
              {data.tableB.map((row, index) => (
                ...
                >
                  <div
                    className={cn(
                      bodyCellClass,
                      "sticky left-0 w-160 bg-neutral-1 font-semibold text-white md:w-270"
                    )}
                  >
                    {row.model}
                  </div>
                  {[row.pmax, row.vmp, row.imp, row.voc, row.isc, row.maxVoltage, row.fuse].map((value, i) => (
                    <div key={i} className={cn(bodyCellClass, "flex-1 text-center")}>
                      {value}
                    </div>
                  ))}
                </div>
