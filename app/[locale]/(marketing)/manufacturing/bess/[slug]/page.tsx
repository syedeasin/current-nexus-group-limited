...
export async function generateMetadata({ params }: RouteParams): Promise<Metadata> {
  ...
  const page = await getPublishedManufacturingPage(ManufacturingCategory.BESS, slug, locale);
  if (!page) return NOT_FOUND_METADATA;
  return buildManufacturingMetadata(page, "bess", locale);
}

export default async function BessSlugPage({ params }: RouteParams) {
  const { locale, slug } = await params;
  const page = await getPublishedManufacturingPage(ManufacturingCategory.BESS, slug, locale);
  if (!page) notFound();
